...
async function Product_createShopify() {
  const qproduct = await query(`CALL sp_search_product_Shopify`);
  if (qproduct.code === 200) {
    if(qproduct.query[0][0].amount !== 0){
...
var job = new CronJob('*/5 * * * * *', () => { app();}, null, true, 'America/Los_Angeles');
job.start();